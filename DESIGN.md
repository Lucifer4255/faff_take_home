...
### 14.6 Implementation outcome (built & verified 10 Jul)

Capture answered every open question above; the adapter (`src/adapters/homeservices/{client,parse,index}.ts`) is built and verified end-to-end against **live** Urban Company through the full harness (interpret → tool loop → EXECUTE gate → approve), guest and no-spend.

- **Endpoints (all `POST www.urbanclap.com/api/v2/…`, guest, no auth token):** `growth/search/discoverySearch` = `search_catalog`; `growth/customerJourney/initiateSeoJourney` = the category page carrying the **earliest-slot** preview. Server-driven-UI `{layout, dataStore}` trees (walk them), gated only on device/version headers (`x-brand-key: urbanCompany`, `x-device-id`, `x-version-*`). Transport is the same **Playwright-as-TLS-vehicle** as Blinkit (`launchIdentity`, `page.evaluate(fetch)`) — headless passes Cloudflare for browse/search. So availability **was clean enough to promote to a direct API** (§10 resolved) — no DOM scraping.
